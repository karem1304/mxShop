                ...
                <div id="aside" class="col-md-3">
                    <form action="{{ route('product', [$id]) }}" method="get">
                        ...
                        <div class="aside">
                            ...
                            <div class="price-filter">
                                ...
                                <div class="input-number price-min">
                                    <input id="price-min1" type="number" name="number_min"
                                        @if ($number_min) value="{{ $number_min }}" @else value="1000" @endif>
                                    <span class="qty-up">+</span>
                                    <span class="qty-down">-</span>
                                </div>
                                <span>-</span>
                                <div class="input-number price-max">
                                    <input id="price-max1" type="number" name="number_max"
                                        @if ($number_max) value="{{ $number_max }}" @else value="50000000" @endif>
                                    <span class="qty-up">+</span>
                                    <span class="qty-down">-</span>
                                </div>
                            </div>
                        ...
                        <div class="aside">
                            ...
                            <div class="checkbox-filter">
                                @foreach ($brands as $brand)
                                    @php
                                        $numberProduct = App\Models\Product::whereHas('category', function ($query) use ($id) {
                                            $query->where('uuid', $id);
                                        })
                                    ...
                                    @if ($numberProduct > 0)
                                        <div class="input-checkbox">
                                            <input type="radio" id="{{ $brand->uuid }}" name="brand"
                                                value="{{ $brand->uuid }}"
                                                @if ($brand_id) @if ($brand_id == $brand->uuid) checked @endif
                                                @endif>
                        ...
                        <button type="submit" class="btn btn-danger col-lg-12"
                            style="margin-bottom: 1.5rem;margin-top: 1.5rem;"> <i class="fa fa-search"></i>
                            Rechercher</button>
                    </form>
                            ...
                            <div class="product-widget">
                                ...
                                <div class="product-body">
                                    ...
                                    <h3 class="product-name"><a
                                            href="{{ route('info', [$product->uuid]) }}">{{ $product->name }}</a></h3>
                                    <h4 class="product-price">{{ format($product->price) }} <del
                                            class="product-old-price">{{ format($product->firstPrice) }}</del></h4>
                                </div>
                            </div>
                            <!-- /product widget -->
                        @endforeach
